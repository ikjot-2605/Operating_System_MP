...
class hrnn_Process // each process will have the following attributes
{
    public:
...
bool hrrn_comp_atime(hrnn_Process P, hrnn_Process Q) //to sort according to arrival time
{
    if(P.atime==Q.atime)
        return P.no<Q.no;
...
int hrrn_main()
{
    ...
    vector<hrnn_Process> p(n);   //creating array for the class
    ...
    sort(p.begin(),p.end(),hrrn_comp_atime); //sorting the Processes according to arrival time
    ...
    while(completed != n)   //
    {
        int idx=-1;
        float hrr=-1;
        //finding the process with HRR among the processes those that have been arrived till now
        for(int i = 0; i < n; i++)
        {
            if(p[i].atime <= current_time && p[i].is_completed == 0)
            {
                float temp = (p[i].btime + (current_time - p[i].atime)) / (float)p[i].btime;
				if (hrr < temp)
                {
					hrr = temp;
                    idx = i;
