// a program to implement MFT with first-fit
...
int main (){
    int total_size, n, i,prs ,p = 0,counter1 = 0,counter2 = 0;
    ...
    for(int j = 0; j < prs; j ++){
        ...
        for(i = 0; i < n; i++){ // checks for the partiton and allots based on first fit method
            if(p == 0){
            if(a[j][0] <= Partition[i][0] && Partition[i][1] == 0  && Partition[j][2] != 0){
                Partition[i][1] = j+1;
                a[j][1] = Partition[i][2];
