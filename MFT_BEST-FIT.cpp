...
int main (){
    int total_size, n, i,prs ,p = 0,counter1 = 0,counter2 = 0,min,k;
    ...
    for(int j = 0; j < prs; j ++){
        ...
        min = y;
        int flag = 0;
        for(i = 0; i < n; i++){ // checks for the partiton and allots based on best fit method
            if(a[j][0] <= Partition[i][0] && min  > (Partition[i][0]-a[j][0]) && Partition[i][1] == 0  && Partition[j][2] != 0){
                min = Partition[i][0]-a[j][0];
                k = i;
                flag = 1;
            }
        }
