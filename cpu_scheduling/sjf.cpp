...
class sjf_Process // each process will have the following attributes
{
    public:
...
bool sjf_comp_atime(sjf_Process P, sjf_Process Q) //to sort according to arrival time
{
    if(P.atime==Q.atime)
        return P.no<Q.no;
...
int sjf_main()
{
    ...
    vector<sjf_Process> p(n);   //creating array for the class
    ...
    sort(p.begin(),p.end(),sjf_comp_atime); //sorting the Processes according to arrival time
    ...
    while(completed != n)   //
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            {
                if(p[i].btime < lowest_btime)
                {
                    lowest_btime = p[i].btime;
                    idx = i;
                }
                if(p[i].btime == lowest_btime)    //if the burst time of two process are same, the process which arrived first will be executed
                {
