...
bool first_allocate_partition(struct process* p) {
    if (p->size > remaining_space) return false;
    struct partition* temp = new struct partition(); int index = -1;

    for (int i=0; i<partitions.size(); i++)
        if (partitions[i]->pid == -1 && partitions[i]->size >= p->size) {
            temp = partitions[i]; index = i; break; }
...
bool first_delete_partition (int n) {
    for (int i=0; i<partitions.size(); i++) {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: // make a new allocation request
                ...
                allocated_partition = first_allocate_partition(temp);
            ...
            case 2: // unallocate an existing process
                ...
                cin >> n;
                if (first_delete_partition(n)) cout << "The process was unallocated." << endl;
