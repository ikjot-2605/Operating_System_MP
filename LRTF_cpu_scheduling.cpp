...
int main()
{
    ...
    while(completed != n)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            {
                if(p[i].burst_remaining > largest_btime)
                {
                    largest_btime = p[i].burst_remaining;
                    idx = i;
                }
                if(p[i].burst_remaining == largest_btime)    //if the burst time of two process are same, the process which arrived first will be executed
                {
