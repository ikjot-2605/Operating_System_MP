            ...
            if (p[processes - 1].req <= left_mem)
            {
                ...
                while (t != NULL)
                {
                    if (t->p_id < 0)
                    {
                        ...
                        {
                            parts++;
                            main_memory[parts - 1].next_partition = t->next_partition;
                            t->next_partition = main_memory + parts - 1;
                            main_memory[parts - 1].partition = t->partition - p[processes - 1].req;
                            t->partition = p[processes - 1].req;
                            main_memory[parts - 1].p_id = -1;
                        }
                        if (t->partition == p[processes - 1].req)
                        {
                            t->p_id = p[processes - 1].p_id;
                            p[processes - 1].position = t;
                            left_mem = left_mem - t->partition;
                            order();
                            printf("process %d is allocted in the partition %d of the main memory \n", processes, t->partition_id);
                            break;
                        }
                    }
                    t = t->next_partition;
